...
import io.github.contractautomata.care.examples.alicebob.exampleWithoutCare.principals.Bob;
...
public class BobImplementation implements Runnable {
                    ...
                    public void run() {
                                ...
                                ObjectOutputStream oout = new ObjectOutputStream(socket.getOutputStream()))
                        {
                            ...
                            while (true) {
                                ...
                                if (action.equals(Orchestrator.check_msg)) {
                                    check(oin, oout);
                                    break;
                                }
                                if (action.equals(Orchestrator.stop_msg))
                                {
                                ...
                                if (action.startsWith(Orchestrator.choice_msg))
                                {
    ...
    public String select(State<String> currentState, String[] toChoose) {
        if (currentState.isFinalState()&&generator.nextInt(2)==0) //50% chance of terminating
            return Orchestrator.stop_choice;
        else
    ...
    private void check(ObjectInputStream oin, ObjectOutputStream oout) throws ClassNotFoundException, IOException {
        ...
        if (orcChoiceType.equals(this.getChoiceType())&&orcActType.equals(this.getActionType()))
            oout.writeObject(Orchestrator.ack_msg);
        else {
